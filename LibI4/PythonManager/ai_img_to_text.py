...
def __load_model__(model_name: str, device: str):
    if (cfg.current_data.use_tf_instead_of_pt):
        return TFAutoModelForCausalLM.from_pretrained(model_name, low_cpu_mem_usage = cfg.current_data.low_cpu_or_memory, trust_remote_code = True)
    else:
        return AutoModelForCausalLM.from_pretrained(model_name, low_cpu_mem_usage = cfg.current_data.low_cpu_or_memory, trust_remote_code = True).to(device)
