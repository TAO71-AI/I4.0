...
def run_server_command(command_data: str, extra_data: dict[str] = {}) -> str:
    ...
    if (command.startswith("ai_response ")):
        return DoPrompt(command[12:len(command)], "", esm, translator, True, conver, use_default_sys_prompts)["response"]
    elif (command.startswith("ai_fresponse ")):
        return str(DoPrompt(command[13:len(command)], "", esm, translator, True, conver, use_default_sys_prompts))
    elif (command.startswith("ai_translation ")):
        return DoPrompt(command[15:len(command)], "-t-ncb", esm, translator, True, conver, use_default_sys_prompts)["response"]
    elif (command.startswith("ai_image ")):
        return DoPrompt(command[9:len(command)], "-ncb-img", esm, translator, True, conver, use_default_sys_prompts)
