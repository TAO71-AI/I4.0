...
def __load_model__(Config: dict[str, any], Index: int) -> tuple[AutoModelForCausalLM, AutoTokenizer, str]:
    ...
    try:
        ...
        if (Config["hf_load_mode"] != None and not Config["hf_load_mode"]):
            # Load in 4-bit
            modelExtraKWargs["load_in_4bit"] = True
        elif (Config["hf_load_mode"] != None and Config["hf_load_mode"]):
            # Load in 8-bit
            modelExtraKWargs["load_in_8bit"] = True
    except:
        # Ignore
        pass
