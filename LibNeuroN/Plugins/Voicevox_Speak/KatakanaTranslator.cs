...
namespace TAO.NeuroN.Plugins.Voicevox
{
    public static class KatakanaTranslator
    {
        ...
        private static string RemoveAccentMarkEtc(string Text)
        {
            return Text.ToLower()
                //a
                .Replace('á', 'a')
        ...
        public static string ENtoJA(string Text)
        {
            ...
            {
                //Vocals
                {"a", "ei"}, {"i", "ai"}, {"y", "uai"}, {"u", "iu"}, {"e", "ii"}, {"o", "ou"},
            };

            Console.WriteLine(EStoJA(TranslateFromDictionary(Text, table, -1)));

            return EStoJA(TranslateFromDictionary(Text, table, -1));
        }
    }
}