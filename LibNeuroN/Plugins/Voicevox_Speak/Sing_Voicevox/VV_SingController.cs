...
using TAO.NeuroN.Plugins.Sing;

namespace TAO.NeuroN.Plugins.Voicevox.Sing
{
                ...
                foreach ((int, string, VV_VoiceData) data in Songs[SongIndex].SongLyrics)
                {
                    ...
                    File.WriteAllBytes("VV_SingPlugin/" + Songs[SongIndex].Name + "/" + data.Item1 + ".wav",
                        Voicevox_ServerConnection.SendToServer(new TParameter[]
                        {
                            ...
                            new TParameter("speed", vdata.Velocity.ToString(), TParamType.GET),
                            new TParameter("velocity", vdata.Velocity.ToString(), TParamType.GET)
                        }, -1).Result);
                }
