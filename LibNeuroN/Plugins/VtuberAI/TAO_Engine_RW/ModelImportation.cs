    ...
    public static class ModelImportation
    {
        ...
        private static (TVector3, TVector2, TVector3) ObjBlockToData(string BlockData, (TVector3[], TVector2[], TVector3[]) Data)
        {
            ...
            for (int i = 0; i < 3; i++)
            {
                if (i == 0)
                {
                    vert = Data.Item1[(int)TBasicMath.StringToFloat(blockVerts[i]) - 1];
                }
                else if (i == 1)
                {
                    texCoord = Data.Item2[(int)TBasicMath.StringToFloat(blockVerts[i]) - 1];
                }
                else if (i == 2)
                {
                    vertNormal = Data.Item3[(int)TBasicMath.StringToFloat(blockVerts[i]) - 1];
                }
            }
        ...
        public static TModelData ObjToTMD(string[] ObjData)
        {
            ...
            List<TVector3> nverts = new List<TVector3>();
            ...
            foreach (string l in ObjData)
            {
                ...
                {
                    nverts.Add(TBasicMath.StringToVector3(ll.Substring(3)));
                }
                ...
                else if (ll.StartsWith("f "))
                {
                    ...
                    foreach (string block in blocks)
                    {
                        (TVector3, TVector2, TVector3) blockData = ObjBlockToData(
                            block,
                            (verts.ToArray(), texCoords.ToArray(), nverts.ToArray())
                        );
