...
prompt_order: str = cb.DefaultOrder()
...
def run_server_command(command_data: str) -> str:
    ...
    if (command.startswith("ai_response ")):
        ...
        processing = True
        ai_response = cb.MakePrompt(command[12:len(command)], prompt_order)["response"]
        processing = False
        ...
        processing = True
        ai_response = cb.MakePrompt(command[13:len(command)], prompt_order)
        processing = False
